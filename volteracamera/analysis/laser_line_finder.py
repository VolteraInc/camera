...
from volteracamera.VolteraCppTools_Python import analyze_laser_image
import logging
...
def reject_outlier(data, m=2, image_edge_threshold=200):
    ...
    indexes = abs(data - np.median(data[indexes_away_from_bottom])
                  ) < m * np.std(data[indexes_away_from_bottom])
...
class LaserLineFinder (object):
    ...
    def process(self, image):
        ...
        laser_points = self.pool.map(
            LaserLineFinder._find_center_point, col_values)
        logging.debug(
            "Laser line finder finished, finding {} points".format(len(laser_points)))
        return laser_points

    def process_cpp(self, image):
        """
        Process the entire image at once by calling a c++ function.
        """
        logging.debug("c++ Laser line finder started.")
        if (len(image.shape) != 2):
            raise RuntimeError("Grayscale images only.")

        laser_points = analyze_laser_image(image)
        logging.debug(
            "Laser line finder finished, finding {} points".format(len(laser_points)))
        return laser_points
            ...
            while (True):
                ...
                if (self.stop_capture == 0):
                    while (self.stop_capture == 0):
                        ...
                        logging.debug(
                            "Captured Image : {}".format(image_count))
                        image_points = finder.process_cpp(image[:, :, 0])
                        image_points_full = [[[i, j]]
                                             for i, j in enumerate(image_points)]
                        intensities = [image[int(ind[0][1]), int(
                            ind[0][0]), 2] for ind in image_points_full]
                        data_points = self.point_projector.project(
                            image_points_full)

                        logging.debug(
                            "{} were found and are being transferred for display.".format(len(data_points)))
                        buffer = ""
                        for point, intensity in zip(data_points, intensities):
                            buffer += "{}, {}, {}, {}, {}\n".format(
                                image_count, point[0], point[1], point[2], intensity)
...
class LaserProcessingClient(object):
    ...
    def get_data(self)->list:
        ...
        if len(lines) == 0:
            return []
        points = [{"x": float(point[1]), "y":float(point[2]) + float(point[0])
                   * PROFILE_OFFSET, "z":float(point[3]), "i":point[4]} for point in lines]
        logging.debug(
            "Laser processing client recieved {} points".format(len(points)))
        return points

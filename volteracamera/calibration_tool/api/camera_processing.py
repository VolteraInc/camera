...
def camera_calibration():
    ...
    if request.method == "GET":
        if 'camera_calibration' not in session:
            session['camera_calibration'] = Undistort(
                np.array([[5000, 0, 1641], [0, 5000, 1232], [0, 0, 1]]), np.zeros(5))
...
def find_camera_position():
    ...
    ############
    # REMOVE 2 FROM THE POSITION WHEN NEW STAGE DATA RECIEVED
    ############
    try:
        return jsonify({"success": True, "message": "", "position": [float(pos[1])/1000.0, 2.0 * float(pos[2])/1000.0, float(pos[3])/1000.0]})
    except:
...
def fit_camera():
    ...
    positions = [[point[0], -point[1], point[2]]
                 for point in json_object['positions']]
    ...
    with open("IntrinsicsPoints.csv", "w") as fid:
        for pos_2d, pos_3d in zip(points_2d, positions):
            fid.write(
                f"{pos_2d[0]}, {pos_2d[1]}, {pos_3d[0]}, {pos_3d[1]}, {pos_3d[2]}\n")
